...
public class MainGUI {
    ...
    private void init() {
        ...
        try {
            java.net.URL res = getClass().getResource("/ui/ppm.png");
            if (res != null) imageIcon = new ImageIcon(res);
        ...
        if (imageIcon != null) frame.setIconImage(imageIcon.getImage());
    ...
    private class StartPanel extends JPanel {
        ...
        StartPanel() {
            ...
            applyBtn.addActionListener(e -> switchCard("APPLY"));
            loginBtn.addActionListener(e -> switchCard("LOGIN"));
            guestDonateBtn.addActionListener(e -> guestDonate());
    ...
    private class ApplyPanel extends JPanel {
        ...
        ApplyPanel() {
            ...
            submit.addActionListener(e -> doApply());
            back.addActionListener(e -> switchCard("START"));
        }
        ...
        private void doApply() {
            ...
            try {
                ...
                //Clear all fields after successful application
                nidF.setText("");
                nameF.setText("");
                emailF.setText("");
                phoneF.setText("");
    ...
    private class LoginPanel extends JPanel {
        ...
        LoginPanel() {
            ...
            backBtn.addActionListener(e -> switchCard("START"));
        }
        ...
        private void doLogin() {
            ...
            //Always clear fields after login attempt
            emailF.setText("");
            passF.setText("");
            if (m == null) {
    ...
    private void donate() {
        ...
        try {
            ...
            switch (currentUser.getRole()) {
                case MEMBER -> memberPanel.refresh();
                case ADMIN -> adminPanel.refresh();
                default -> leaderPanel.refresh();
            }
    ...
    private void vote() {
        ...
        switch (level) {
            case CENTRAL -> cmap = system.getCentralCommittee().getElection().getAllCandidates();
            case DIVISIONAL -> cmap = system.getDivisionalCommittee(division).getElection().getAllCandidates();
            case DISTRICT -> cmap = system.getDistrictCommittee(district).getElection().getAllCandidates();
            default -> {
        ...
        switch (level) {
            case CENTRAL ->
                ok = system.getCentralCommittee().getElection().vote(currentUser, role, cand);
            case DIVISIONAL ->
                ok = system.getDivisionalCommittee(division).getElection().vote(currentUser, role, cand);
            case DISTRICT ->
                ok = system.getDistrictCommittee(district).getElection().vote(currentUser, role, cand);
            default ->
                ok = false;
        }
    ...
    private void approveMember() {
        ...
        for (int i = 0; i < pend.size(); i++) {
            Member m = pend.get(i);
            checkBoxes[i] = new JCheckBox(m.getName() + " (" + m.getNationalId() + ")");
    ...
    private void terminateMember() {
        ...
        if (choice == 0) {
            ...
            member = system.findById(nid.trim());
            if (member == null) {
                member = system.findByEmail(nid.trim());
            }
        } else if (choice == 1) {
    ...
    private String profile(Member m) {
        return "Name: " + m.getName() + "\n"
                ...
                + "Division: " + (m.getAddress() != null ? m.getAddress().getDivision() : "N/A") + "\n"
                + "District: " + (m.getAddress() != null ? m.getAddress().getDistrict() : "N/A") + "\n"
